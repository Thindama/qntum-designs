...
const PLANS = {
  free: {
    ...
    price: 0,
    tokens_limit: 500000,
    features: ['1 Projekt', '500K Tokens/Monat', 'Community Support', 'Qntum Subdomain']
  },
  pro: {
    name: 'Pro',
    priceMonthly: 29,
    ...
    features: ['Unbegrenzte Projekte', '8M Tokens/Monat', 'Alle Skills', 'Custom Domain', 'Priority Support', 'Opus 4 Zugang']
  },
  business: {
    name: 'Business',
    priceMonthly: 99,
    ...
    features: ['Alles aus Pro', '50M Tokens/Monat', 'Team-Zugang (bald)', 'API-Zugang (bald)', 'Dedicated Support']
  }
};
...
router.post('/checkout', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const session = await s.checkout.sessions.create({
      ...
      success_url: `${req.protocol}://${req.get('host')}/dashboard.html?payment=success`,
...
router.get('/subscription', requireAuth, async (req, res) => {
  try {
    ...
    res.json({
      ...
      tokens_limit: profile.tokens_limit || PLANS.free.tokens_limit,
      features: PLANS[profile.plan || 'free']?.features || PLANS.free.features
    });
  } catch (err) {
